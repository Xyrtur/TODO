import 'package:flutter/material.dart';
import 'package:flutter_svg/svg.dart';
import 'package:intl/intl.dart';
import 'package:todo/blocs/cubits.dart';
import 'package:todo/blocs/daily_todo_bloc.dart';
import 'package:todo/blocs/monthly_todo_bloc.dart';
import 'package:todo/blocs/unfinished_bloc.dart';
import 'package:todo/models/event_data.dart';
import 'package:todo/utils/centre.dart';
import 'package:todo/widgets/event_text_field.dart';
import 'package:calendar_date_picker2/calendar_date_picker2.dart';
import 'package:todo/widgets/svg_button.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import '../../utils/datetime_ext.dart';

import 'custom_time_picker.dart' as custom_time_picker;
import 'delete_confirmation_dialog.dart';

class AddEventDialog extends StatelessWidget {
  final bool daily;
  final DateTime monthOrDayDate;
  final Map<dynamic, EventData>? dailyTableMap;
  final List<dynamic>? orderedDailyKeyList;
  final EventData? event;
  ValueNotifier<List<DateTime?>?> dateResults = ValueNotifier<List<DateTime?>?>(null);
  ValueNotifier<TimeRangeState> timeRangeChosen = ValueNotifier<TimeRangeState>(TimeRangeState(null, null));
  ValueNotifier<bool?> deleting = ValueNotifier<bool?>(false);
  AddEventDialog.daily(
      {super.key,
      this.daily = true,
      this.event,
      required this.monthOrDayDate,
      required this.dailyTableMap,
      required this.orderedDailyKeyList});
  AddEventDialog.monthly(
      {super.key,
      this.daily = false,
      this.event,
      required this.monthOrDayDate,
      this.dailyTableMap,
      this.orderedDailyKeyList});

  @override
  Widget build(BuildContext context) {
    dateResults.addListener(() {
      context.read<DialogDatesCubit>().update(dateResults.value);
    });
    timeRangeChosen.addListener(() {
      context.read<TimeRangeCubit>().update(timeRangeChosen.value.startResult, timeRangeChosen.value.endResult);
    });
    deleting.addListener(() {
      if (deleting.value ?? false) Navigator.pop(context);
    });

    TextEditingController controller = TextEditingController(text: event?.text);
    return AlertDialog(
      scrollable: true,
      contentPadding:
          EdgeInsets.symmetric(horizontal: Centre.safeBlockHorizontal * 5, vertical: Centre.safeBlockVertical * 3),
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(40))),
      backgroundColor: Centre.dialogBgColor,
      elevation: 5,
      content: SizedBox(
        height: daily ? Centre.safeBlockVertical * 35 : Centre.safeBlockVertical * 48,
        width: Centre.safeBlockHorizontal * 85,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
              children: [
                Padding(
                  padding: EdgeInsets.only(left: Centre.safeBlockHorizontal * 4),
                  child: Text(
                    event == null ? "New Event" : "Edit Event",
                    style: Centre.todoSemiTitle,
                  ),
                ),
                event != null
                    ? GestureDetector(
                        onTap: () async {
                          deleting.value = await showDialog<bool>(
                              context: context,
                              builder: (BuildContext context) {
                                return DeleteConfirmationDialog(
                                    type: daily ? DeletingFrom.todoTable : DeletingFrom.monthCalen, event: event!);
                              });
                        },
                        child: SizedBox(
                          height: Centre.safeBlockVertical * 3.5,
                          width: Centre.safeBlockVertical * 3.5,
                          child: Icon(
                            Icons.delete_rounded,
                            color: Color(event!.color),
                          ),
                        ),
                      )
                    : const SizedBox(
                        height: 0,
                        width: 0,
                      )
              ],
            ),
            Padding(
              padding: EdgeInsets.only(
                  top: Centre.safeBlockVertical * 1,
                  left: Centre.safeBlockHorizontal * 2,
                  right: Centre.safeBlockHorizontal * 2),
              child: const Divider(
                color: Colors.grey,
              ),
            ),
            Padding(
              padding: EdgeInsets.symmetric(vertical: Centre.safeBlockVertical * 1),
              child: Row(
                mainAxisSize: MainAxisSize.max,
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  for (int i = 0; i < 5; i++)
                    GestureDetector(
                      onTap: () {
                        context.read<ColorCubit>().update(i);
                      },
                      child: Container(
                        width: 25,
                        height: 25,
                        decoration: BoxDecoration(
                            color: Centre.colors[i],
                            border: Border.all(color: Colors.white, width: 1.5),
                            borderRadius: const BorderRadius.all(Radius.circular(40))),
                        child: context.read<ColorCubit>().state == i
                            ? Icon(
                                Icons.check,
                                size: Centre.safeBlockHorizontal * 5,
                                color: Colors.white,
                              )
                            : null,
                      ),
                    )
                ],
              ),
            ),
            Row(
              mainAxisSize: MainAxisSize.max,
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children: [
                for (int i = 5; i < 10; i++)
                  GestureDetector(
                    onTap: () {
                      context.read<ColorCubit>().update(i);
                    },
                    child: Container(
                      width: 25,
                      height: 25,
                      decoration: BoxDecoration(
                          color: Centre.colors[i],
                          border: Border.all(color: Colors.white, width: 1.5),
                          borderRadius: const BorderRadius.all(Radius.circular(40))),
                      child: context.read<ColorCubit>().state == i
                          ? Icon(
                              Icons.check,
                              size: Centre.safeBlockHorizontal * 5,
                              color: Colors.white,
                            )
                          : null,
                    ),
                  )
              ],
            ),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  EventNameTextField(controller: controller),
                  Row(
                    children: !daily
                        ? [
                            Container(
                              margin: EdgeInsets.symmetric(horizontal: Centre.safeBlockHorizontal * 3),
                              height: Centre.safeBlockHorizontal * 43,
                              width: Centre.safeBlockVertical * 8,
                              decoration: const BoxDecoration(
                                  color: Color.fromARGB(255, 77, 77, 77),
                                  borderRadius: BorderRadius.all(Radius.circular(10))),
                              child: Column(
                                children: [
                                  GestureDetector(
                                    onTap: () {
                                      if (event == null) {
                                        context.read<CalendarTypeCubit>().pressed(CalendarType.single);
                                        context.read<DialogDatesCubit>().update(null);
                                      }
                                    },
                                    child: svgButton(
                                        name: "single_date",
                                        color: event == null
                                            ? (context.read<CalendarTypeCubit>().state == CalendarType.single
                                                ? Centre.yellow
                                                : Centre.colors[4])
                                            : (context.read<CalendarTypeCubit>().state == CalendarType.single
                                                ? Centre.yellow
                                                : Centre.lighterDialogColor),
                                        height: 7,
                                        width: 7,
                                        padding: EdgeInsets.all(Centre.safeBlockHorizontal),
                                        borderColor: event == null &&
                                                context.read<CalendarTypeCubit>().state == CalendarType.single
                                            ? Centre.colors[8]
                                            : Colors.transparent),
                                  ),
                                  GestureDetector(
                                    onTap: () {
                                      if (event == null) {
                                        context.read<CalendarTypeCubit>().pressed(CalendarType.ranged);
                                        context.read<DialogDatesCubit>().update(null);
                                      }
                                    },
                                    child: svgButton(
                                        name: "range_date",
                                        color: event == null
                                            ? (context.read<CalendarTypeCubit>().state == CalendarType.ranged
                                                ? Centre.yellow
                                                : Centre.colors[4])
                                            : (context.read<CalendarTypeCubit>().state == CalendarType.ranged
                                                ? Centre.yellow
                                                : Centre.lighterDialogColor),
                                        height: 7,
                                        width: 7,
                                        padding: EdgeInsets.all(Centre.safeBlockHorizontal),
                                        borderColor: event == null &&
                                                context.read<CalendarTypeCubit>().state == CalendarType.ranged
                                            ? Centre.colors[8]
                                            : Colors.transparent),
                                  ),
                                  GestureDetector(
                                    onTap: () {
                                      if (event == null) {
                                        context.read<CalendarTypeCubit>().pressed(CalendarType.multi);
                                        context.read<DialogDatesCubit>().update(null);
                                      }
                                    },
                                    child: svgButton(
                                        name: "multi_date",
                                        color: event == null
                                            ? (context.read<CalendarTypeCubit>().state == CalendarType.multi
                                                ? Centre.yellow
                                                : Centre.colors[4])
                                            : Centre.lighterDialogColor,
                                        height: 7,
                                        width: 7,
                                        padding: EdgeInsets.all(Centre.safeBlockHorizontal),
                                        borderColor: event == null &&
                                                context.read<CalendarTypeCubit>().state == CalendarType.multi
                                            ? Centre.colors[8]
                                            : Colors.transparent),
                                  )
                                ],
                              ),
                            ),
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Row(
                                  children: [
                                    GestureDetector(
                                      onTap: () async {
                                        dateResults.value = await showCalendarDatePicker2Dialog(
                                          dialogBackgroundColor: Centre.dialogBgColor,
                                          barrierColor: Colors.transparent,
                                          borderRadius: 40,
                                          context: context,
                                          config: CalendarDatePicker2WithActionButtonsConfig(
                                            weekdayLabelTextStyle: Centre.todoText.copyWith(color: Centre.yellow),
                                            controlsTextStyle: Centre.dialogText,
                                            gapBetweenCalendarAndButtons: 0,
                                            lastMonthIcon: const SizedBox(
                                              width: 0,
                                              height: 0,
                                            ),
                                            nextMonthIcon: const SizedBox(
                                              width: 0,
                                              height: 0,
                                            ),
                                            shouldCloseDialogAfterCancelTapped: true,
                                            cancelButtonTextStyle: Centre.dialogText,
                                            okButton: Container(
                                              margin: EdgeInsets.only(right: Centre.safeBlockHorizontal * 3),
                                              child: Text(
                                                "OK",
                                                style: Centre.dialogText,
                                              ),
                                            ),
                                            dayTextStyle: Centre.todoText,
                                            calendarType: context.read<CalendarTypeCubit>().state == CalendarType.single
                                                ? CalendarDatePicker2Type.single
                                                : context.read<CalendarTypeCubit>().state == CalendarType.ranged
                                                    ? CalendarDatePicker2Type.range
                                                    : CalendarDatePicker2Type.multi,
                                            firstDate: DateTime(monthOrDayDate.year, monthOrDayDate.month, 1),
                                            lastDate: DateTime(monthOrDayDate.year, monthOrDayDate.month + 1, 0),
                                            currentDate: DateTime.now(),
                                            selectedDayHighlightColor: Centre.red,
                                          ),
                                          dialogSize:
                                              Size(Centre.safeBlockHorizontal * 85, Centre.safeBlockVertical * 48),
                                          initialValue: context.read<DialogDatesCubit>().state ?? [],
                                        );
                                      },
                                      child: BlocBuilder<CalendarTypeCubit, CalendarType>(
                                        builder: (context, state) {
                                          return svgButton(
                                            name: state == CalendarType.single
                                                ? "single_date"
                                                : state == CalendarType.ranged
                                                    ? "range_date"
                                                    : "multi_date",
                                            color: Centre.yellow,
                                            height: 7,
                                            width: 7,
                                            margin: EdgeInsets.symmetric(horizontal: Centre.safeBlockHorizontal * 2),
                                            padding: EdgeInsets.all(Centre.safeBlockHorizontal),
                                          );
                                        },
                                      ),
                                    ),
                                    Builder(builder: (context) {
                                      final calendarState = context.watch<CalendarTypeCubit>().state;
                                      final dateResultsState = context.watch<DialogDatesCubit>().state;
                                      return Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          mainAxisSize: MainAxisSize.min,
                                          children: calendarState != CalendarType.ranged
                                              ? [
                                                  SizedBox(
                                                      width: Centre.safeBlockHorizontal * 21,
                                                      child: Text(
                                                        calendarState == CalendarType.single
                                                            ? (dateResultsState?[0] != null
                                                                ? DateFormat('MMM d').format(dateResultsState![0]!)
                                                                : "")
                                                            : "",
                                                        maxLines: 3,
                                                        overflow: TextOverflow.ellipsis,
                                                        softWrap: true,
                                                        style: Centre.dialogText,
                                                      )),
                                                ]
                                              : [
                                                  SizedBox(
                                                      width: Centre.safeBlockHorizontal * 21,
                                                      child: Text(
                                                        dateResultsState?[0] != null
                                                            ? DateFormat('MMM d').format(dateResultsState![0]!)
                                                            : "",
                                                        maxLines: 3,
                                                        overflow: TextOverflow.ellipsis,
                                                        softWrap: true,
                                                        style: Centre.dialogText,
                                                      )),
                                                  SizedBox(
                                                      width: Centre.safeBlockHorizontal * 21,
                                                      child: Text(
                                                        dateResultsState?[1] != null
                                                            ? DateFormat('MMM d').format(dateResultsState![1]!)
                                                            : "",
                                                        maxLines: 3,
                                                        overflow: TextOverflow.ellipsis,
                                                        softWrap: true,
                                                        style: Centre.dialogText,
                                                      )),
                                                ]);
                                    }),
                                  ],
                                ),
                                SizedBox(
                                  height: Centre.safeBlockVertical * 2,
                                ),
                                Builder(builder: (context) {
                                  final calendarState = context.watch<CalendarTypeCubit>().state;
                                  final checkBoxState = context.watch<CheckboxCubit>().state;
                                  final TimeRangeState timeRangeState = context.watch<TimeRangeCubit>().state;

                                  return calendarState != CalendarType.ranged
                                      ? Row(
                                          children: [
                                            GestureDetector(
                                                onTap: () async {
                                                  if (checkBoxState) {
                                                    timeRangeChosen.value = await chooseTimeRange(
                                                        context: context, daily: daily, editingEvent: event);
                                                  }
                                                },
                                                child: svgButton(
                                                  name: "range_time",
                                                  color: Centre.yellow,
                                                  height: 7,
                                                  width: 7,
                                                  margin:
                                                      EdgeInsets.symmetric(horizontal: Centre.safeBlockHorizontal * 2),
                                                  padding: EdgeInsets.all(Centre.safeBlockHorizontal),
                                                )),
                                            Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text(
                                                  "${timeRangeState.startResult?.hour == null ? timeRangeState.startResult!.hour.toString().padLeft(2, '0') : ""}${timeRangeState.startResult?.minute == null ? timeRangeState.startResult!.minute.toString().padLeft(2, '0') : ""}",
                                                  style: Centre.dialogText.copyWith(
                                                      decoration: checkBoxState ? TextDecoration.lineThrough : null),
                                                ),
                                                Text(
                                                  "${timeRangeState.endResult?.hour == null ? timeRangeState.endResult!.hour.toString().padLeft(2, '0') : ""}${timeRangeState.endResult?.minute == null ? timeRangeState.endResult!.minute.toString().padLeft(2, '0') : ""}",
                                                  style: Centre.dialogText.copyWith(
                                                      decoration: checkBoxState ? TextDecoration.lineThrough : null),
                                                ),
                                              ],
                                            )
                                          ],
                                        )
                                      : const SizedBox(
                                          height: 0,
                                          width: 0,
                                        );
                                }),
                                BlocBuilder<CalendarTypeCubit, CalendarType>(builder: (context, state) {
                                  return state != CalendarType.ranged
                                      ? Row(
                                          children: [
                                            BlocBuilder<CheckboxCubit, bool>(
                                              builder: (context, state) {
                                                return GestureDetector(
                                                  onTap: () {
                                                    context.read<CheckboxCubit>().toggle();
                                                  },
                                                  child: Container(
                                                    margin: EdgeInsets.only(
                                                        left: Centre.safeBlockHorizontal * 6,
                                                        right: Centre.safeBlockHorizontal),
                                                    height: Centre.safeBlockHorizontal * 6,
                                                    width: Centre.safeBlockHorizontal * 6,
                                                    decoration: BoxDecoration(
                                                        border: Border.all(width: 2, color: Centre.colors[4]),
                                                        borderRadius: BorderRadius.all(Radius.circular(3)),
                                                        color: Colors.transparent),
                                                    child: state
                                                        ? Center(
                                                            child: Icon(
                                                            Icons.check,
                                                            color: Centre.textColor,
                                                            size: Centre.safeBlockHorizontal * 4,
                                                          ))
                                                        : const SizedBox(
                                                            height: 0,
                                                            width: 0,
                                                          ),
                                                  ),
                                                );
                                              },
                                            ),
                                            Text(
                                              "Full day",
                                              style: Centre.todoText,
                                            )
                                          ],
                                        )
                                      : const SizedBox(
                                          height: 0,
                                          width: 0,
                                        );
                                }),
                              ],
                            ),
                            editButton(height: 20, width: 15, context: context, oldEvent: event)
                          ]
                        : [
                            GestureDetector(
                                onTap: () async {
                                  timeRangeChosen.value = await chooseTimeRange(
                                      context: context,
                                      daily: daily,
                                      dailyDate: monthOrDayDate,
                                      orderedDailyKeyList: orderedDailyKeyList,
                                      dailyTableMap: dailyTableMap,
                                      editingEvent: event);
                                },
                                child: svgButton(
                                    name: "range_time",
                                    color: Centre.yellow,
                                    height: 7,
                                    width: 7,
                                    margin: EdgeInsets.symmetric(horizontal: Centre.safeBlockHorizontal * 2),
                                    padding: EdgeInsets.all(Centre.safeBlockHorizontal))),
                            BlocBuilder<TimeRangeCubit, TimeRangeState>(
                              builder: (context, state) {
                                return Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(
                                      "${state.startResult?.hour == null ? state.startResult!.hour.toString().padLeft(2, '0') : ""}${state.startResult?.minute == null ? state.startResult!.minute.toString().padLeft(2, '0') : ""}",
                                      style: Centre.dialogText,
                                    ),
                                    Text(
                                      "${state.endResult?.hour == null ? state.endResult!.hour.toString().padLeft(2, '0') : ""}${state.endResult?.minute == null ? state.endResult!.minute.toString().padLeft(2, '0') : ""}",
                                      style: Centre.dialogText,
                                    ),
                                  ],
                                );
                              },
                            ),
                            editButton(height: 7, width: 15, context: context, oldEvent: event)
                          ],
                  ),
                ],
              ),
            ),
          ],
        ),
      ),
    );
  }

  Widget editButton(
      {required int height, required int width, required BuildContext context, required EventData? oldEvent}) {
    return GestureDetector(
      onTap: () {
        TimeOfDay start = context.read<TimeRangeCubit>().state.startResult!;
        TimeOfDay end = context.read<TimeRangeCubit>().state.endResult!;

        if (oldEvent == null) {
          if (!daily) {
            bool fullDay = context.read<CheckboxCubit>().state;

            // TODO: do checks to ensure all needed info is there
            switch (context.read<CalendarTypeCubit>().state) {
              case CalendarType.single:
                context.read<MonthlyTodoBloc>().add(MonthlyTodoCreate(
                    selectedDailyDay: context.read<DateCubit>().state,
                    event: EventData(
                        fullDay: fullDay,
                        start: context
                            .read<DialogDatesCubit>()
                            .state![0]!
                            .add(!fullDay ? Duration(hours: start.hour, minutes: start.minute) : Duration()),
                        end: context
                            .read<DialogDatesCubit>()
                            .state![0]!
                            .add(!fullDay ? Duration(hours: end.hour, minutes: end.minute) : Duration()),
                        color: Centre.colors[context.read<ColorCubit>().state].value,
                        text: "controller", //TODO: set up controlling the text here
                        finished: false)));
                break;
              case CalendarType.ranged:
                context.read<MonthlyTodoBloc>().add(MonthlyTodoCreate(
                    selectedDailyDay: context.read<DateCubit>().state,
                    event: EventData(
                        fullDay: true,
                        start: context.read<DialogDatesCubit>().state![0]!,
                        end: context.read<DialogDatesCubit>().state![1]!,
                        color: Centre.colors[context.read<ColorCubit>().state].value,
                        text: "controller", //TODO: set up controlling the text here
                        finished: false)));
                break;
              case CalendarType.multi:
                for (DateTime? date in context.read<DialogDatesCubit>().state ?? []) {
                  context.read<MonthlyTodoBloc>().add(MonthlyTodoCreate(
                      selectedDailyDay: context.read<DateCubit>().state,
                      event: EventData(
                          fullDay: fullDay,
                          start: date!.add(!fullDay ? Duration(hours: start.hour, minutes: start.minute) : Duration()),
                          end: date.add(!fullDay ? Duration(hours: end.hour, minutes: end.minute) : Duration()),
                          color: Centre.colors[context.read<ColorCubit>().state].value,
                          text: "controller", //TODO: set up controlling the text here
                          finished: false)));
                }

                break;
            }
          } else {
            context.read<TodoBloc>().add(TodoCreate(
                event: EventData(
                    fullDay: false,
                    start: context.read<DateCubit>().state.add(Duration(hours: start.hour, minutes: start.minute)),
                    end: context.read<DateCubit>().state.add(Duration(hours: end.hour, minutes: end.minute)),
                    color: Centre.colors[context.read<ColorCubit>().state].value,
                    text: "controller", //TODO: set up controlling the text here
                    finished: false)));
          }
        } else {
          if (!daily) {
            bool fullDay = context.read<CheckboxCubit>().state;

            // TODO: do checks to ensure all needed info is there
            switch (context.read<CalendarTypeCubit>().state) {
              case CalendarType.single:
                context.read<MonthlyTodoBloc>().add(MonthlyTodoUpdate(
                    oldEvent: oldEvent,
                    selectedDailyDay: context.read<DateCubit>().state,
                    event: event!.edit(
                        fullDay: fullDay,
                        start: context
                            .read<DialogDatesCubit>()
                            .state![0]!
                            .add(!fullDay ? Duration(hours: start.hour, minutes: start.minute) : Duration()),
                        end: context
                            .read<DialogDatesCubit>()
                            .state![0]!
                            .add(!fullDay ? Duration(hours: end.hour, minutes: end.minute) : Duration()),
                        color: Centre.colors[context.read<ColorCubit>().state].value,
                        text: "controller", //TODO: set up controlling the text here
                        finished: false)));
                break;
              case CalendarType.ranged:
                context.read<MonthlyTodoBloc>().add(MonthlyTodoUpdate(
                    oldEvent: oldEvent,
                    selectedDailyDay: context.read<DateCubit>().state,
                    event: event!.edit(
                        fullDay: true,
                        start: context.read<DialogDatesCubit>().state![0]!,
                        end: context.read<DialogDatesCubit>().state![1]!,
                        color: Centre.colors[context.read<ColorCubit>().state].value,
                        text: "controller", //TODO: set up controlling the text here
                        finished: false)));
                break;
              default:
                break;
            }
          } else {
            if (oldEvent.start.isSameDate(other: context.read<DateCubit>().state, daily: true)) {
              context.read<TodoBloc>().add(TodoUpdate(
                  event: event!.edit(
                      fullDay: false,
                      start: context.read<DateCubit>().state.add(Duration(hours: start.hour, minutes: start.minute)),
                      end: context.read<DateCubit>().state.add(Duration(hours: end.hour, minutes: end.minute)),
                      color: Centre.colors[context.read<ColorCubit>().state].value,
                      text: "controller", //TODO: set up controlling the text here
                      finished: false)));
            } else {
              context.read<TodoBloc>().add(TodoAddUnfinished(
                  event: event!.edit(
                      fullDay: false,
                      start: context.read<DateCubit>().state.add(Duration(hours: start.hour, minutes: start.minute)),
                      end: context.read<DateCubit>().state.add(Duration(hours: end.hour, minutes: end.minute)),
                      color: Centre.colors[context.read<ColorCubit>().state].value,
                      text: "controller", //TODO: set up controlling the text here
                      finished: false)));
              context.read<UnfinishedListBloc>().add(const UnfinishedListUpdate());
            }
          }
        }

        Navigator.pop(context);
      },
      child: Expanded(
        child: SizedBox(
          height: Centre.safeBlockVertical * height,
          width: Centre.safeBlockHorizontal * width,
          child: Align(
            alignment: Alignment.bottomRight,
            child: SvgPicture.asset(
              "assets/icons/edit.svg",
              color: Centre.colors[3],
              width: Centre.safeBlockHorizontal * 10,
              height: Centre.safeBlockHorizontal * 10,
            ),
          ),
        ),
      ),
    );
  }
}

Future<TimeRangeState> chooseTimeRange(
    {required BuildContext context,
    required bool daily,
    EventData? editingEvent,
    DateTime? dailyDate,
    Map<dynamic, EventData>? dailyTableMap,
    List<dynamic>? orderedDailyKeyList}) async {
  TimeOfDay? endResult;
  TimeOfDay? startResult;
  double time = 0;
  do {
    time = 0;
    startResult = await showDialog(
        context: context,
        builder: (BuildContext context) {
          return custom_time_picker.TimePickerDialog(
            initialTime: startResult ?? TimeOfDay.now(),
            helpText: "Choose Start time",
            daily: daily,
            orderedDailyKeyList: orderedDailyKeyList,
            dailyTableMap: dailyTableMap,
            editingEvent: editingEvent,
            dailyDate: dailyDate,
          );
        });
    if (startResult != null) {
      endResult = await showDialog(
          context: context,
          builder: (BuildContext context) {
            return custom_time_picker.TimePickerDialog(
              initialTime: startResult!.replacing(minute: (startResult.minute + 15) % 60),
              helpText: "Choose End time",
              daily: daily,
              orderedDailyKeyList: orderedDailyKeyList,
              dailyTableMap: dailyTableMap,
              startTime: startResult,
              editingEvent: editingEvent,
              dailyDate: dailyDate,
            );
          });
      time = endResult != null ? endResult.hour + endResult.minute / 60.0 : 0;
    }
  } while (time == 1 / 60.0);
  return TimeRangeState(startResult, endResult);
}
