...
class MonthYearPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final PageController pc = PageController(
        initialPage: context.read<YearTrackingCubit>().state - 2020); //Only let events from 2020 onwards
    ...
    Widget monthBtn(int monthNum, DateTime selectedMonth, int pageIndex) {
      return GestureDetector(
          onTap: () {
            context
                .read<MonthDateCubit>()
                .update(DateTime.utc(context.read<YearTrackingCubit>().state, monthNum));
