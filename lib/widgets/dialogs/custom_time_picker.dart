              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    Expanded(
                        child: _HourControl(
                            badbad: badHour, fragmentContext: fragmentContext)),
                    _StringFragment(timeOfDayFormat: timeOfDayFormat),
                    Expanded(
                        child: _MinuteControl(
                            badbad: badMinute,
...
class _HourMinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ShapeBorder shape = RoundedRectangleBorder(
        side: BorderSide(
            color: badNotGood
                ? Color.fromARGB(255, 202, 107, 102)
                : Colors.transparent,
...
class _TimePickerDialogState extends State<TimePickerDialog>
  ...
  void _handleOk() {
      ...
      if (widget.startTime != null) {
        if ((widget.startTime!.minute + 15) - 60 > 0 &&
            !(newHour > (widget.startTime!.hour + 1) % 24 ||
                newHour == widget.startTime!.hour + 1 &&
                    newMinute >= (widget.startTime!.minute + 15) % 60)) {
          badMinute = true;
          setState(() {
        ...
        } else if (newHour == widget.startTime!.hour &&
            newMinute < widget.startTime!.minute + 15) {
          badMinute = true;
          setState(() {
            widget.errorInvalidText = "Events must be minimum 15 minutes";
          });
    ...
    if (widget.daily) {
      Duration localTimeDiff = DateTime(widget.dailyDate!.year,
              widget.dailyDate!.month, widget.dailyDate!.day, 7, 0)
          .timeZoneOffset;
          ...
          if (widget.startTime == null) {
            ...
            for (EventData v in widget.dailyTableMap!.values) {
              start = widget.dailyDate!.add(Duration(
                  hours: newHour >= 0 && newHour <= 1 ? newHour + 24 : newHour,
                  minutes: newMinute));
            ...
            for (EventData v in widget.dailyTableMap!.values) {
              end = widget.dailyDate!.add(Duration(
                  hours: newHour >= 0 && newHour <= 1 ? newHour + 24 : newHour,
                  minutes: newMinute));
              start = widget.dailyDate!.add(Duration(
                  hours:
                      widget.startTime!.hour >= 0 && widget.startTime!.hour <= 1
          ...
          if (widget.startTime == null) {
            ...
            for (EventData event in widget.dailyTableMap!.values) {
              ...
              if (start
                      ...
                      .isInTimeRange(event.start, event.end) ||
                  possibleEnd
                      .subtract(localTimeDiff)
                      .isInTimeRange(event.start, event.end) ||
                  start.subtract(localTimeDiff).enclosesOrContains(
                      possibleEnd.subtract(localTimeDiff),
                      event.start,
                      event.end)) {
                badHour = true;
                badMinute = true;
                setState(() {
                  widget.errorInvalidText =
                      "Will clash with event ${event.text}";
                });
                return;
              }
            }
            ...
            for (EventData event in widget.dailyTableMap!.values) {
              ...
              if (start
                      ...
                      .isInTimeRange(event.start, event.end) ||
                  start.subtract(localTimeDiff).enclosesOrContains(
                      end.subtract(localTimeDiff), event.start, event.end)) {
                badHour = true;
                badMinute = true;
                setState(() {
                  widget.errorInvalidText =
