...
class FutureTodoTile extends StatefulWidget {
  ...
  const FutureTodoTile({super.key, required this.todo, required this.textController, required this.focusNode});
...
class _FutureTodoTileState extends State<FutureTodoTile> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    animController = AnimationController(duration: const Duration(milliseconds: 300), vsync: this);
    arrowController = AnimationController(duration: const Duration(milliseconds: 100), vsync: this);
    ...
    animation = CurvedAnimation(parent: animController, curve: Curves.fastOutSlowIn)
  ...
  Widget tileBtn({required IconData icon, required Color color, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, color: color, size: Centre.safeBlockHorizontal * 5.5),
      ),
    );
  }
          ...
          child: GestureDetector(
              ...
              if (widget.todo.expandable) {
                ...
                if (arrowController.status == AnimationStatus.dismissed ||
                  ...
                  for (
                    ...
                    i < futureList.length && futureList[widget.todo.index].indented < futureList[i].indented;
                    i++
                  ) {
            ...
            onDoubleTap: () {
              ...
              if (widget.todo.indented > 0) {
                ...
                for (
                  int i = currentWidgetIndex + 1;
                  i < futureList.length && futureList[currentWidgetIndex].indented < futureList[i].indented;
                  i++
                ) {
                ...
                if (futureList[currentWidgetIndex - 1].indented == widget.todo.indented &&
                    (currentWidgetIndex + 1 == futureList.length ||
                        futureList[indexAfterAllOfList].indented <= futureList[currentWidgetIndex - 1].indented)) {
                  ...
                  child: Row(
                    children: [
                      ...
                      GestureDetector(
                        onDoubleTap: () {
                          ...
                          if (widget.todo.indented < 4 &&
                            ...
                            while (indexTapped != -1 && futureList[indexTapped].indented != widget.todo.indented) {
                              indexTapped--;
                            }
                      ...
                      Expanded(
                        child: !editingState
                            ? Padding(
                                padding: EdgeInsets.symmetric(vertical: Centre.safeBlockVertical * 1.2),
                                child: Text(
                                  widget.todo.text,
                                  maxLines: 2,
                                  textHeightBehavior: const TextHeightBehavior(
                                      ...
                                      ? TextFormField(
                                          decoration: InputDecoration(
                                            ...
                                            focusedBorder: UnderlineInputBorder(
                                              borderSide: BorderSide(color: Centre.primaryColor),
                                            ),
                                          ),
                                    ...
                                    (indexEditing ?? -1) == widget.todo.index
                                        ...
                                        : tileBtn(
                                            ...
                                            onTap: () {
                                              ...
                                              List<FutureTodo> futureList = context
                                                  .read<FutureTodoBloc>()
                                                  .state
                                                  .futureList;
                                              ...
                                              context.read<TodoTileAddCubit>().update([
                                                widget.todo.index + 1,
                                                widget.todo.indented + 1,
                                                0,
                                              ]);
                                              context.read<FutureTodoBloc>().add(
                                                FutureTodoListUpdate(eventList: futureList),
                                              );
                                            },
                                          ),
                                    tileBtn(
